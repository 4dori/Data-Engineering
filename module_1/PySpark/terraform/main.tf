...
terraform {
  backend "azurerm" {
    # added credentials
    storage_account_name = "spark4basic"
    container_name       = "spark"
    key                  = "key1"
    access_key = "6fXHRWxbZejDG7YFL7/S04EzPATJ4oQcLHN1CSceM8sloYqyPTCCxcnP8xXzRTDzPxIfL8iRS9TP+AStInwAVw==" # have to change it
  }
}
...
resource "azurerm_kubernetes_cluster" "bdcc" {
  ...
  default_node_pool {
    name       = "default"
    node_count = 1
    vm_size    = "Standard_D8_v3" # changed vm
  }
